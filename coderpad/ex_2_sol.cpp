...
std::vector<int> find_quadruplet_sum_fast(const std::vector<int>& numbers, int target) {
    // Build a map of every pair sum -> one (val_a, val_b) pair that achieves it.
    // Since the same number may be reused, we allow i==j (same index used twice).
    std::unordered_map<int, std::pair<int,int>> pair_sums;
    int n = numbers.size();
    for (int i = 0; i < n; ++i) {
        for (int j = i; j < n; ++j) {          // j starts at i, not i+1, to allow reuse
            int s = numbers[i] + numbers[j];
            pair_sums.emplace(s, std::make_pair(numbers[i], numbers[j]));
        }
    }

    // For every pair (i, j), check whether target - that sum is already stored.
    for (int i = 0; i < n; ++i) {
        for (int j = i; j < n; ++j) {
            int current_sum = numbers[i] + numbers[j];
            int complement   = target - current_sum;
            auto it = pair_sums.find(complement);
            if (it != pair_sums.end()) {
                return {numbers[i], numbers[j], it->second.first, it->second.second};
            }
        }
    }
    return {};
...
void run_testcase(const std::vector<int>& numbers, int target, const std::string& testcase_name) {
    ...
    std::cout << "PASSED (" << elapsed << "s)" << std::endl;
